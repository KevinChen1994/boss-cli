...
| `/web/chat/index` | 沟通列表、候选人会话页 | `boss list [--unread] [--filter <all\|unread\|resume-acquired>]`、`boss chat <姓名>`、`boss chat [姓名] --index <序号> [--unread] [--filter <all\|unread\|resume-acquired>]`、`boss send`、`boss action`、`boss download-resume --out <目录>` | `list` / `chat` 会先检查当前位置，不在该页时直接进入；`--unread` 与 `--filter` 互斥；`all` 与 `unread` 固定使用顶部“全部”Tab，`resume-acquired` 使用顶部“已获取简历”Tab；`chat --index` 必须使用与 `list` 相同的筛选参数解释 1-based 序号，同时提供姓名时会校验序号对应候选人姓名；`send` / `action` / `download-resume` 要求当前会话已打开；`download-resume` 下载当前会话中最新的附件简历，目标目录必须已存在且可写。 |
