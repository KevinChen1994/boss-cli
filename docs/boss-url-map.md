...
| `/web/chat/index` | 沟通列表、候选人会话页 | `boss list [--unread] [--filter <all\|unread\|new-greeting-unread\|resume-acquired>]`、`boss chat <姓名>`、`boss chat [姓名] --index <序号> [--unread] [--filter <all\|unread\|new-greeting-unread\|resume-acquired>]`、`boss send`、`boss action`、`boss download-resume --out <目录>` | `list` / `chat` 会先检查当前位置，不在该页时直接进入；`--unread` 与 `--filter` 互斥；`all` 与 `unread` 使用顶部“全部”Tab，`new-greeting-unread` 使用“新招呼”Tab 的“未读”筛选，`resume-acquired` 使用“已获取简历”Tab；列表输出包含从 `data-id` 解析的 `BOSS_ID`；`chat --index` 必须使用与 `list` 相同的筛选参数解释 1-based 序号；`send` / `action` / `download-resume` 要求当前会话已打开。 |
